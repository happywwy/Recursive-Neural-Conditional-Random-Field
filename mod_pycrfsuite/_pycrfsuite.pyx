...
'''
#create class for converting pointer to ndarray
...
cdef class ArrayWrapper:
    ...
    cdef set_data(self, int size, void* data_ptr):
        ...
        This cannot be done in the constructor as it must receieve C-level
        arguments.
        Parameters:
        -----------
        size: int
...
cdef class BaseTrainer(object):
    ...
    def __cinit__(self):
        ...
        self.c_trainer.select("l2sgd", "crf1d")
    ...
    def modify(self, xseq, int num):
        self.c_trainer.modify(to_seq(xseq), num)
    ...
    '''
    '''
    def train(self, model, np.ndarray[double, ndim=1, mode="c"] weight, int w_size, \
        np.ndarray[double, ndim=1, mode="c"] delta, int inst, double time, double decay, \
        int check, int size, int holdout=-1):
    '''
    def train(self, model, np.ndarray[double, ndim=1, mode="c"] weight, \
        np.ndarray[double, ndim=1, mode="c"] delta, int inst, double eta, double decay, \
        np.ndarray[double, ndim=1, mode="c"] loss, int check, int holdout=-1):
        '''
        ...
        if check == 0:
            #self.c_trainer.train(model, holdout, &weight[0], w_size, &delta[0], inst, time, decay, check, size)
            self.c_trainer.train(model, holdout, &weight[0], &delta[0], inst, eta, decay, &loss[0], check)
        else:
            #array = self.c_trainer.train(model, holdout, &weight[0], w_size, inst, time, decay, check, size)
            #status_code = self.c_trainer.train(model, holdout, &weight[0], w_size, &delta[0], inst, time, decay, check, size)
            status_code = self.c_trainer.train(model, holdout, &weight[0], &delta[0], inst, eta, decay, &loss[0], check)
            '''
    ...
    '''
    #wwy add new function wrapper        
    def train_init(self, int holdout=-1):
        self._before_train()
        status_code = self.c_trainer.train_init(holdout)
        if status_code != crfsuite_api.CRFSUITE_SUCCESS:
            raise CRFSuiteError(status_code)
    '''
    def params(self):
